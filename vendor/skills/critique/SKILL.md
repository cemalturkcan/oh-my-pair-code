...
- Only run the detector if `impeccable` is already available locally or the user explicitly asked for detector-backed results.
...
- For large directories (200+ scannable files), use `--fast`.
...
1. Create a new tab or page for this assessment. Do not reuse the tab from Assessment A.
2. Navigate to the relevant route.
3. Inspect the result with the available browser observation tools (page state, text, accessibility snapshot, console, screenshots when useful).
4. Record visible anti-patterns, hierarchy issues, and likely detector-equivalent findings directly from those observations.
...
- **Suggested skill**: Which skill could address this next (from: `animate`, `quieter`, `shape`, `optimize`, `adapt`, `clarify`, `layout`, `distill`, `delight`, `audit`, `harden`, `polish`, `bolder`, `typeset`, `critique`, `colorize`, `overdrive`)
...
- Only recommend skills from: `animate`, `quieter`, `shape`, `optimize`, `adapt`, `clarify`, `layout`, `distill`, `delight`, `audit`, `harden`, `polish`, `bolder`, `typeset`, `critique`, `colorize`, `overdrive`
