...
3. **Run the Teach Mode below (REQUIRED)**: If neither source has context, gather it now with the Teach Mode in this skill before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
When the user explicitly asks for a shape-then-build frontend flow, follow the [craft flow](reference/craft.md) and use the requested feature as the feature description.

---

## Teach Mode

When the user explicitly asks to set up or refresh design context, skip all design work above and instead run the teach flow below. This is a one-time setup that gathers design context for the project.
...
When the user explicitly asks to extract reusable components, patterns, or tokens into the design system, follow the [extract flow](reference/extract.md) and use the requested area as the extraction target.
