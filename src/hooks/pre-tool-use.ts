      ...
      if (
        ...
        if (blocked) {
          ...
          const msg = isTaskBlocked
            ? `[PlanMode] task tool is blocked in planning mode. For read-only workers (killua, ginko, rust), use delegate instead of task. The user will /go to start execution.`
            : count >= 3
